...
fulltitle: QuickViewer オフィシャルページ
# other options
---
...
画像データの読み込みを先回りして行い、読み込んだ画像データをキャッシュしておくことで、これまでにない軽快さで画像を閲覧できるようになります。
...
- **01net.com telecharger.com** (フランス語)
...
- ウィンドウの表示方法の切り替え
    ...
    - 常に手前に表示
    - ウィンドウの表示を次回起動時に復元する
- [縮小フィルター](shader-ja) による表示品質の切り替え
    - バイリニア補間(標準)
    - 最近傍補間
    - バイキュービック補間、Spline16・Spline36補間 (CPU処理)
...
縮小フィルターを切り替えると画像の描画品質が変わります。詳しくは [縮小フィルター](shader-ja) にて。
