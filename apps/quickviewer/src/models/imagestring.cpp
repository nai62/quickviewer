...
QString ImageString::formatString(QString fmt)
{
    ...
    for (int i = 0; i < fmt.length(); i++) {
        ...
        case 'f': {
            ...
            // Exact image file size in bytes, e.g. '1,154,340 bytes'
        case 'F':
            result << QString(tr("%L1 Bytes")).arg(page->fileSize);
            break;
            // Decoded image size in a human-readable format, e.g. '1.59 MB'
        case 'b': {
        ...
        case 'n': {
            ...
            // Separator before the second image in 2-page spread view
        case '2': {
            if (pageCount < 2) {
                i = fmt.length();
            }
