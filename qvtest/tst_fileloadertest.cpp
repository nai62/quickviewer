...
private Q_SLOTS:
    ...
    void testCase0_formatClassifiers();
    ...
    void testCase11_recursiveDirectoryTraversal();
    void testCase12_emptyDirectoryContentsAreStable();
};
...
void FileLoaderTest::testCase0_formatClassifiers()
{
    // Specialized checks must not depend on isImageFile() being called first.
    ...
    // Match a real extension, not an arbitrary filename suffix.
...
void FileLoaderTest::testCase11_recursiveDirectoryTraversal()
{
    QTemporaryDir temporaryDir;
    QVERIFY(temporaryDir.isValid());
    ...
    FileLoaderDirectory recursive(
        nullptr, root.path(), FileLoaderDirectory::TraversalMode::Recursive);
    ...
    QCOMPARE(normalized,
             QStringList({"003.jpe", "chapter/002.jpe", "chapter/section/001.jpe"}));
}

void FileLoaderTest::testCase12_emptyDirectoryContentsAreStable()
{
    QTemporaryDir temporaryDir;
    QVERIFY(temporaryDir.isValid());
