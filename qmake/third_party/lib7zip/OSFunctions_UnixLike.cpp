...
HMODULE Load7ZLibrary(const wstring & name)
{
  ...
  // Prefer the 7-Zip engine staged beside QuickViewer. dlopen() with a bare
  // name can otherwise select an older system p7zip before the bundled one.
  std::string executable_current = getExecutableDir();
  string bundledPath = executable_current + "/" + tmpName;
  HMODULE pHandler = dlopen(bundledPath.c_str(), RTLD_LAZY | RTLD_GLOBAL);

  if (pHandler)
    return pHandler;

  pHandler = dlopen(tmpName.c_str(), RTLD_LAZY | RTLD_GLOBAL);
